...
private val CLIENT_ID_REGEX = Regex("\"@client_id\"\\s*:\\s*(\\d+)")
private val REQUEST_ID_REGEX = Regex("\"@extra\"\\s*:\\s*\"?(\\d+)")
...
internal class TdlEngine(
    ...
    init {
        ...
        coroutineScope.launch(context = coroutineDispatcherReceiver) {
            ...
            while (true) {
                val json = native.receive(timeoutInSeconds = MAX_TIMEOUT) ?: continue
                // Local fix: a malformed object from TDLib must not kill the receiver
                // loop (and the process). Pending requests get a synthetic Error;
                // undeserializable updates are dropped.
                val triple = try {
                    deserializer.deserialize(json = json)
                } catch (throwable: Throwable) {
                    val clientId = CLIENT_ID_REGEX.find(json)?.groupValues?.get(1)?.toIntOrNull()
                    val requestId = REQUEST_ID_REGEX.find(json)?.groupValues?.get(1)?.toLongOrNull()
                    if (clientId == null || requestId == null || requestId == 0L) {
                        continue
                    }
                    val error = dev.g000sha256.tdl.dto.Error(
                        code = 500,
                        message = "TDL deserialization failed: ${throwable.message ?: throwable::class.simpleName}",
                    )
                    Triple(first = clientId, second = requestId, third = error)
                }
